...
`createMediaPipeWorkerPoseAdapter()` accepts one transferable `ImageBitmap` or `VideoFrame` estimate at a time and requires the exact capture timestamp paired with that frame. It reports the actual accepted transfer frame type plus actual worker location and the configured delegate only after Pose Landmarker creation succeeds, performs no silent fallback, closes accepted frames in the worker, rejects/closes concurrent submissions, and disposes the task/worker terminally.

The experiment uses the official plain-JavaScript IIFE in a classic worker with an injectable, version-pinned Tasks Vision bundle URL. Tasks Vision 1.0.1 also supports an ES module worker, but only when `FilesetResolver.forVisionTasks(wasmPath, true)` selects `vision_wasm_module_internal`; omitting the flag or passing `false` selects the classic loader and fails in a module worker with exact `ModuleFactory not set.`. A real Chromium CPU/GPU and ImageBitmap/VideoFrame matrix corrected the earlier false conclusion that module workers were intrinsically unsupported. Classic execution remains a valid supported choice for this bounded adapter, not a fallback claim. Self-host the IIFE bundle alongside the injectable WASM/model assets for release use. CPU-WASM and GPU-WebGL provider truth still requires actual browser/device proof; requested configuration alone is never reported as an active provider.
